...
""" 0. construct mesh, accuracy of mesh"""
mesh_obj, el_pos = mesh.create(8, h0=0.08)
...
""" Object setup, Anomaly would be the object/array with resistance/Impedance got from measurement. """
mesh_obj['alpha'] = np.random.rand(tri.shape[0]) * 200 + 100
anomaly = [{'x': 0.1, 'y': 0.1, 'd': 0.1, 'perm': 1000.0}]
mesh_new = mesh.set_perm(mesh_obj, anomaly=anomaly)

""" 2. FEM simulation """
el_dist, step = 8, 1
ex_mat = eit_scan_lines(8, el_dist)
